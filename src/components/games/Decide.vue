<template>
    <div class="w-100 overflow-hidden">
        ...
        <div id="decide-options" class="mt-3 mx-auto px-5 w-75 d-flex justify-content-between align-items-center">
            <svg @click="(e) => checkMatch(e, { mode: game.mode, isAnswer: game.isAnswer, handleClick: game.handleClick }, false)"
                ...
                class="bi bi-check-lg text-success" data-answer="true" viewBox="0 0 16 16">
                <path
                    d="M12.736 3.97a.733.733 0 0 1 1.047 0c.286.289.29.756.01 1.05L7.88 12.01a.733.733 0 0 1-1.065.02L3.217 8.384a.757.757 0 0 1 0-1.06.733.733 0 0 1 1.047 0l3.052 3.093 5.4-6.425z" />
            </svg>
            <svg @click="(e) => checkMatch(e, { mode: game.mode, isAnswer: game.isAnswer, handleClick: game.handleClick }, false)"
                role="button" xmlns="http://www.w3.org/2000/svg" width="50" height="50" fill="currentColor"
                class="bi bi-x text-danger" data-answer="false" viewBox="0 0 16 16">
                <path
...
const word = ref(game.questions[game.currentQuestIndex].word);
...
watch(() => game.currentQuestIndex, (newIndex, oldIndex) => {
    ...
    setTimeout(() => {
        word.value = game.questions[oldIndex].word;
        wordElm.style.transition = 'none';
        wordElm.style.transform = 'translateX(-500px)';
    }, 500);
    setTimeout(() => {
        textColor.value = getRandomColor();
        word.value = game.questions[newIndex].word;
