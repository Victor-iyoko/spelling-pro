<template>
    <div class="d-flex justify-content-between align-items-center mb-2 w-100">
        <span class="d-flex align-items-center user-select-none opacity-0 fs-4">
            {{ index + 1 }}
        </span>
        <span @click="checkMatch" class="option fs-2 user-select-none" role="button" :style="{ color: color }">
            {{ title }}
        </span>
...
function checkMatch(e) {
    const isAnswer = game.isAnswer(props.title, true);
    // prevent user from vlivking options rapidly
    e.target.classList.add("pe-none");
    // it holds the options parent container
    const optionsParent = e.target.parentNode.parentNode.children;
    ...
    if (isAnswer) {
        ...
        for (let i = 0; i < optionsParent.length; i++) {
            // removing line-through class from all the words
            optionsParent[i].children[1].classList.remove("text-decoration-line-through");
        }
        setTimeout(() => {
            ...
            for (let i = 0; i < optionsParent.length; i++) {
                // removing line-through class from all the words
                optionsParent[i].children[1].classList.remove("pe-none");
            }
            // set new question
    ...
    game.isAnswer(props.title, false);
}
</script>
